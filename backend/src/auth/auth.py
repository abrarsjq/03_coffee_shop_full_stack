...
AUTH0_DOMAIN = 'coffeechop.us.auth0.com'
ALGORITHMS   = ['RS256']
...
def get_token_auth_header():
    ...
    auth_splite = auth.split()

    if len(auth_splite) != 2 :
        raise AuthError({
                "code": "unauthorized",
                "description": "Token must be in the format `Bearer token` "
            }, 401)

    elif auth_splite[0].lower() != "bearer":
        raise AuthError({
                "code": "unauthorized",
                "description": "Token must be start with `Bearer` "
            }, 401)

    return auth_splite[-1]
